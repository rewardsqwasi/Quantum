...
export default class ShopPage extends BasePage {
  ...
  private filter_button = '//button/span[contains(text(),"Filteren")]';
  ...
  private product_points = '//div[contains(@class,"product-details")]//span[contains(text(),"Beloningspunten")]';
  private add_to_basket_btn = '//span[contains(text(),"Toevoegen")]/parent::a';
  private success_toast = '//div[@class="toast-message" and text()="Het product is succesvol toegevoegd aan je basket"]';
  ...
  getProductDetail(){
    ...
    .then($el => {
      productDetails.points = $el.text().replace("Beloningspunten:", "").trim();
