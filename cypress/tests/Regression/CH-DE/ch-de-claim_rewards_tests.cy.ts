...
describe('CH (DE lang) Region - Claim Rewards Tests', () => {
  ...
  it('Verify Zalando Page', () => {
    ...
    app.claimRewardsPage.rewardDetailElement().each((element) => {
      app.wrap(element).xpath('//a[text()="Vorgang abbrechen"]').should('be.visible');
    });
    app.claimRewardsPage.rewardDetailElement().each((element) => {
      app.wrap(element).xpath('//button[@id="claim" and text()="fordern"]').should('be.visible');
    });
    app.claimRewardsPage.rewardDetailElement().each((element) => {
      app.wrap(element).find('select').should('be.visible').each((element) => {
        app.wrap(element).find('option').each((option, index) => {
          if (index === 0) {
            cy.wrap(option).should('have.text', 'Wählen Sie den Betrag');
