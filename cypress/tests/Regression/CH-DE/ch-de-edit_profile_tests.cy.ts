...
describe('CH (DE lang) Region - Edit Profile Tests', () => {
  let data;
  before(() => {
    ...
    if(IS_PROD==="true"){
      app.loadFixture('prod/ch.json').then((d) => {
        data = d;
      });
    }
    else{
      app.loadFixture('stage/ch.json').then((d) => {
        data = d;
      });
    }
  });
  ...
  it('Verify Page Title', () => {
    app.getPageTitle().should('eq','VELUX PRIMA Portal - Profil bearbeiten');
  });
