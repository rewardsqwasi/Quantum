...
describe('SE Region - Partners Tests', () => {
  ...
  it('Verify Partners Elements', () => {
    ...
    app.partnersPage.partnersDivElement().each((element) => {
      app.wrap(element).xpath('//span[contains(text(),"Lös in")]/parent::button').should('be.visible');
    });
    app.partnersPage.partnersDivElement().each((element) => {
      app.wrap(element).xpath('//span[contains(text(),"Visa detaljer")]/parent::a').should('be.visible');
    });
    app.partnersPage.partnersDivElement().each((element) => {
      app.wrap(element).find('select').should('be.visible').each((element) => {
        app.wrap(element).find('option').each((option, index) => {
          if (index === 0) {
            cy.wrap(option).should('have.text', ' Välj ett värde');
            cy.wrap(option).should('have.attr', 'disabled');
          }
          else {
            const expectedValue = ['500','1000'];
            const expectedText = ['500 SEK', '1000 SEK'];
