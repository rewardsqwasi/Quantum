...
describe('CH (IT lang) Region - Purchase Status Tests', () => {
  ...
  it('Verify Page Title', () => {
    app.getPageTitle().should('eq','VELUX PRIMA Portal - Status Richieste');
  });
  ...
  it('Verify the status selector drop-down filter is working and showing required values', () => {
    ...
    .each((option, index) => {
      const expectedText = ['Tutto', 'In attesa di', 'Respinto', 'Confermato', 'Ricevute di credito']
  ...
  it('Verify if user is able to select the status filter and results are displayed accordingly', () => {
    app.purchaseStatusPage.selectStatus('Respinto').should('have.attr', 'class').and('contain', "responsive-tabs__list__item--active");;
    app.purchaseStatusPage.statusColumnElement().each((element) => {
      app.wrap(element).should('have.text', 'Respinto');
   });
  });
