...
describe('CH (IT lang) Region - Claim Rewards Tests', () => {
  ...
  it('Verify Page Title', () => {
    app.getPageTitle().should('eq','VELUX PRIMA Portal - Riscatta punti');
  });
  ...
  it('Verify Zalando Page', () => {
    ...
    app.claimRewardsPage.rewardDetailElement().each((element) => {
      app.wrap(element).xpath('//a[text()="Cancella"]').should('be.visible');
    });
    app.claimRewardsPage.rewardDetailElement().each((element) => {
      app.wrap(element).xpath('//button[@id="claim" and text()="Richiedi"]').should('be.visible');
    });
    app.claimRewardsPage.rewardDetailElement().each((element) => {
      app.wrap(element).find('select').should('be.visible').each((element) => {
        app.wrap(element).find('option').each((option, index) => {
          if (index === 0) {
            cy.wrap(option).should('have.text', 'Selezionare la somma');
