...
describe('CH (IT lang) Region - Login Tests', () => {
  ...
  it('Verify Press Enter key without adding any field values for email and password.', () => {
    ...
    app.loginPage.getEmailFieldErrorMsg().should('eq', 'Inserisci il tuo indirizzo e-mail');
    app.loginPage.getPasswordFieldErrorMsg().should('eq', 'Inserisci la password');
  });
  ...
  it('Verify Enter Invalid Credentails (Email and Password)', () => {
    ...
    app.loginPage.getToastMessagText().should('eq', 'I tuoi dati di accesso non corrispondono.');
  });
