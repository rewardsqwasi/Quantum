...
describe('NO Region - Login Tests', () => {
  ...
  it('Verify Footer Contact Section', () => {
    ...
    app.loginPage.getAddressFooterText().should('eq', 'VELUX Norge ASGjerdrumsvei 10 G0401 Oslo');
  });
  ...
  it('Verify placeholders for email and password.', () => {
    app.loginPage.emailFieldElement().should('have.attr', 'placeholder', 'Skriv inn e-post adresse');
    app.loginPage.passwordFieldElement().should('have.attr', 'placeholder', 'Oppgi passord');
  });
  ...
  it('Verify Enter Invalid Credentails (Email and Password)', () => {
    ...
    app.loginPage.getToastMessagText().should('eq', 'Innloggingsdetaljene stemmer ikke. Vennligst prøv igjen.');
  });
