...
describe('UK Region - Purchase Status Tests', () => {
  ...
  it('Verify the status selector drop-down filter is working and showing required values', () => {
    ...
    .each((option, index) => {
      const expectedValue = ['All', 'Pending', 'inProgress', 'Approved', 'Rejected', 'Uploading images'];
      const expectedText = ['All', 'Pending', 'In Progress\n', 'Approved\n', 'Rejected\n', 'Uploading Images\n']
  ...
  it('Verify if user is able to select the status filter and results are displayed accordingly', () => {
    app.purchaseStatusPage.selectStatus('Rejected');
    ...
    app.purchaseStatusPage.statusColumnElement().each((element) => {
      app.wrap(element).should('have.text', 'Rejected');
    });
  });
