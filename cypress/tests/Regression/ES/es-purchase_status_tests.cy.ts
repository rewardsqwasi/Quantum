...
describe('ES Region - Purchase Status Tests', () => {
  ...
  it('Verify Page Title', () => {
    app.getPageTitle().should('eq','VELUX Premia - Estado de factura');
  });
  ...
  it('Verify the status selector drop-down filter is working and showing required values', () => {
    ...
    .each((option, index) => {
      ...
      const expectedText = ['Todos', 'Pendiente', 'En Progreso\n', 'Aprobado\n', 'Rechazado\n']
  ...
  it('Verify if user is able to select the status filter and results are displayed accordingly', () => {
    app.purchaseStatusPage.selectStatus('Rechazado');
    ...
    app.purchaseStatusPage.statusColumnElement().each((element) => {
      app.wrap(element).should('have.text', 'Rechazado');
    });
  });
