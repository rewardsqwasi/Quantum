...
describe('ES Region - Partners Tests', () => {
  ...
  it('Verify Partners Elements', () => {
    ...
    app.partnersPage.partnersDivElement().each((element) => {
      app.wrap(element).xpath('//span[contains(text(),"Solicitar")]/parent::button').should('be.visible');
    });
    app.partnersPage.partnersDivElement().each((element) => {
      app.wrap(element).xpath('//span[contains(text(),"Ver los detalles")]/parent::a').should('be.visible');
    });
    app.partnersPage.partnersDivElement().each((element) => {
      app.wrap(element).find('select').should('be.visible').each((element) => {
        app.wrap(element).find('option').each((option, index) => {
          if (index === 0) {
            cy.wrap(option).should('have.text', ' Elegir');
            cy.wrap(option).should('have.attr', 'disabled');
          }
          else {
            const expectedValue = ['5', '10', '15', '20', '25', '50', '100', '250'];
            const expectedText = ['5€', '10€', '15€', '20€', '25€', '50€', '100€', '250€'];
