...
describe('BE (FR lang) Region - Login Tests', () => {
  ...
  it('Verify placeholders for email and password.', () => {
    ...
    app.loginPage.passwordFieldElement().should('have.attr', 'placeholder', 'Broche codée');
  });
  ...
  it('Verify Press Enter key without adding any field values for email and password.', () => {
    ...
    app.loginPage.getPasswordFieldErrorMsg().should('eq', 'Saisissez votre code pin de chiffres');
    app.loginPage.getEmailFieldErrorMsg().should('eq', 'Veuillez saisir votre adresse e-mail');
  });
  ...
  it('Verify Enter Invalid Credentails (Email and Password)', () => {
    ...
    app.loginPage.getToastMessagText().should('eq', 'Les identifiants de connexion sont incorrects. Veuillez essayer à nouveau.');
  });
