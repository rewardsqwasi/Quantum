...
describe('CH (FR lang) Region - Edit Profile Tests', () => {
  let data;
  before(() => {
    ...
    if(IS_PROD==="true"){
      app.loadFixture('prod/ch-fr.json').then((d) => {
        data = d;
      });
    }
    else{
      app.loadFixture('stage/ch-fr.json').then((d) => {
        data = d;
      });
    }
  });
  ...
  it('Verify Page Title', () => {
    app.getPageTitle().should('eq','VELUX PRIMA Portal - Modifier le profil');
  });
