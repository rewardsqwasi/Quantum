...
describe('PT Region - Basket Tests', () => {
  let data;
  before(() => {
    ...
    if(IS_PROD==="true"){
      ...
      app.loadFixture('prod/pt.json').then((d) => {
        data = d;
      });
    }
    else{
      app.loadFixture('stage/pt.json').then((d) => {
        data = d;
      });
    }
    app.loginPage.open(region);
  ...
  it('Verify Delivery Address Section', () => {
    ...
    app.basketPage.countryFieldElement().should('have.value', "Portugal");
  });
