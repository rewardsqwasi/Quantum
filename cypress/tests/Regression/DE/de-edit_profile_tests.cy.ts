...
describe('DE Region - Edit Profile Tests', () => {
  let data;
  before(() => {
    ...
    if(IS_PROD==="true"){
      app.loadFixture('prod/de.json').then((d) => {
        data = d;
      });
    }
    else{
      app.loadFixture('stage/de.json').then((d) => {
        data = d;
      });
    }
  });
  ...
  it('Verify Page Title', () => {
    app.getPageTitle().should('eq','VELUX PLUS - Profil bearbeiten');
  });
