...
describe("Edit Profile", () => {
    ...
    beforeEach(() => {
      ...
      it('should Verify the edit profile page is accessible from header.', () => {
        ...
        .then(() => {
          ...
          cy.contains('Profiel').click({force:true}); // Replace 'Your Po...' with the text of the hidden element
        });
    ...
    cy.go('back');
    cy.get(addPurchaseInvoiceLocators.facebook).click({force: true});
    cy.go('back');
    cy.get(addPurchaseInvoiceLocators.twitter).click({force: true});
    cy.go('back');
    cy.get(addPurchaseInvoiceLocators.youtube).click({force: true});
    cy.go('back');
    cy.get(addPurchaseInvoiceLocators.pinterest).click({force: true});
